...
class TennisGame:
    ...
    def score(self):
        if (self.player1_score < 4 and self.player2_score < 4) and (self.player1_score + self.player2_score < 6):
            ...
            return advantage_player + ("All" if (self.player1_score == self.player2_score) else score_value[self.player2_score])
        else:
            if self.player1_score == self.player2_score:
                return "Deuce"
            advantage_player = self.player1Name if self.player1_score > self.player2_score else self.player2Name
            score = "Win for " if abs(self.player1_score - self.player2_score) > 1 else "Advantage "
